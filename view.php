...
if ($id) {
    ...
    $moduleinstance = $DB->get_record('mod_room', array('id' => $cm->instance), '*', MUST_EXIST);
} else if ($r) {
    $moduleinstance = $DB->get_record('mod_room', array('id' => $n), '*', MUST_EXIST);
...
$event->add_record_snapshot('mod_room', $moduleinstance);
$event->trigger();
